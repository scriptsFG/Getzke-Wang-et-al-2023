...
colors<-c("HK"="#8B8C89", "WT" = "#000001", "sypc"="#009B7D")
...
#read 
read_tsv(file = "Fig3B_data.txt", skip_empty_rows = T, )->data
data[,1:6]->data
na.omit(data)->data
...
data %>%
  filter(SynCom=="no SC")->data2

#########boxplot
ggplot(data2,aes(x=R401, y=Value_log2))+
  geom_jitter(aes(x=R401, y=Value_log2, color=R401), width = 0.3, alpha=0.6)+
  ...
  scale_x_discrete(limits=c("HK", "WT", "sypc"), labels=c("HK", "WT", "sypc"))+
  scale_y_continuous(limits=c(-2.5,6.5))+
  ylab("log2(shoot fresh weight)")+
  xlab("")+
  facet_grid(~NaCl)+
  NULL
